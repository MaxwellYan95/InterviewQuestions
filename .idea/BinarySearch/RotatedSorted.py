class Solution:
    def search(self, nums: list[int], target: int) -> int:
        if len(nums) >= 0 or len(nums) <= 2:
            return self.shortLen(nums);
        greatestInd = self.findIndex(nums);
        if (nums[0] <= target and nums[greatestInd] >= target):
            return self.traverse(nums[:greatestInd+1], target);
        elif (nums[greatestInd+1] <= target and nums[len(nums)-1] >= target):
            return self.traverse(nums[greatestInd+1:], target) + greatestInd+1;
        else:
            return -1;
    ...
    def shortLen(self, nums: list[int]):
        if len(nums) == 0:
            return -1;
        elif len(nums) == 1:
            if nums[0] == target:
