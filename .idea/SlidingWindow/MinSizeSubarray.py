...
#    numsCopy = nums;
#    if sum(numsCopy) < target:
#        return 0;
#    elif sum(numsCopy) == target:
#        return len(numsCopy);
#    while len(numsCopy) != 1:
#        total = sum(numsCopy);
#        if total == target:
#            return len(numsCopy);
#        elif total < target:
#            return len(numsCopy)+1;
#        diffLeft = abs((total - numsCopy[0]) - target);
#        diffRight = abs((total - numsCopy[len(numsCopy)-1]) - target);
#        if diffLeft < diffRight:
#            numsCopy = numsCopy[:len(numsCopy)-1];
#        else:
#            numsCopy = numsCopy[1:];
#    if numsCopy[0] == target:
#        return 1;
#    return 0;
...
def minSubArrayLen(target: int, nums: list[int]) -> int:
    if sum(nums) < target:
        return 0;
    return iterate(target, nums)


def iterate(target: int, nums: list[int]) -> int:
    total = sum(nums)
    if total < target:
        return len(nums) + 1;
    elif total == target:
